...
class CopsAndThiefDevu {
    ...
    void solve(int TC) throws Exception {
        ...
        for(int i = 0; i < M; i++) {
            int start = cops[i] - cover;
            if(start < 0) start = 1;
            int end = cops[i] + cover;
            if(end > 100) end = 100;
        ...
        // Calculate houses that cops cannot search
        int count = 0;
        for(int i = 1; i <= 100; i++) {
            if(houses[i] == 0) count++;
        }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new CopsAndThiefDevu().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new CopsAndThiefDevu().run();
    }
