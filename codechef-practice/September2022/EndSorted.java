...
class EndSorted {
    ...
    void solve(int TC) throws Exception {
        ...
        if(first - 1 > 0 && N - last > 0 && last < first) {
            totalMoves += -1; // Remove the common move.
        }
        println(totalMoves);
    }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new EndSorted().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new EndSorted().run();
    }
