...
class SimpleStatistics {
    ...
    void solve(int TC) throws Exception {
        ...
        Arrays.sort(A);
        // Remove smallest K numbers
        for(int i = 0; i < K; i++) {
            A[i] = 0;
        }

        // Remove largest K numbers
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new SimpleStatistics().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new SimpleStatistics().run();
    }
