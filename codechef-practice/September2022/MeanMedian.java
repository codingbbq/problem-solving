...
class MeanMedian {
    ...
    void solve(int TC) throws Exception {
        int X = ni(), Y = ni();
        // Since Y can always be a Median, we can also make A as Y and Find C as 3X - 2Y
        println(Y + " " + Y + " " + (3*X - 2*Y));
    }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new MeanMedian().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new MeanMedian().run();
    }
