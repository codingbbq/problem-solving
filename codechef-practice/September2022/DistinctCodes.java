...
class DistinctCodes {
    ...
    void solve(int TC) throws Exception {
        String s = ns();
        println(findDistinct(s));
    }

    public int findDistinct(String s) {
        Set<String> c = new HashSet<>();
        for(int i = 1; i < s.length(); i++) {
            c.add(s.charAt(i-1) + "" + s.charAt(i));
        }
        return c.size();
    }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new DistinctCodes().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new DistinctCodes().run();
    }
