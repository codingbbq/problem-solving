...
class Balloons {
    ...
    void solve(int TC) throws Exception {
        ...
        Set<Integer> vibgyor = new HashSet<>();
        int prob = 0;
        for(int i = 0; i < N; i++) {
            if(A[i] <= 7) {
                vibgyor.add(A[i]);
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new Balloons().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new Balloons().run();
    }
