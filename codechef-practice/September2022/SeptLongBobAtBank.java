...
class SeptLongBobAtBank {

    void solve(int TC) throws Exception {
        int W = ni(), X = ni(), Y = ni(), Z = ni();
        println(W + ((X - Y)*Z));
    }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new SeptLongBobAtBank().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new SeptLongBobAtBank().run();
    }
