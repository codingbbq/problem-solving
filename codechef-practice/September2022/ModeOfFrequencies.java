// https://www.codechef.com/submit/MODEFREQ
// Mode of Frequencies
...
class ModeOfFrequencies {
    ...
    void solve(int TC) throws Exception {
        ...
        for(int i=1;i<=10;i++) freqOfFreq[freq[i]]++;

        int ans=1,max=freqOfFreq[1];
        for(int i=2;i<=N;i++) //Finding the mode
        {
            if(freqOfFreq[i]>max)
            {
                max=freqOfFreq[i];
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new ModeOfFrequencies().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new ModeOfFrequencies().run();
    }
