...
class SeptLongSpeedTest {

    void solve(int TC) throws Exception {
        double A = nd(), B = nd(), X = nd(), Y = nd();
        if(A/X == B/Y) {
            println("Equal");
        } else if(A/X < B/Y) {
            println("Bob");
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new SeptLongSpeedTest().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new SeptLongSpeedTest().run();
    }
