...
class VaccineDistribution {
    ...
    void solve(int TC) throws Exception {
        ...
        int days = 0;
        days += (int) Math.ceil((double)risk / D);
        days += (int) Math.ceil((double)(N - risk) / D);
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new VaccineDistribution().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new VaccineDistribution().run();
    }
