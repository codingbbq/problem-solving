...
class BallBox {
    ...
    void solve(int TC) throws Exception {
        int N = ni(), K = ni();
        // We can put sequence of balls in boxes.
        // i.e. 1 in box1, 2 in box2 and so on.
        long sum = (K * (K + 1))/2;
        println((sum <= N) ? "YES" : "NO");
    }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new BallBox().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new BallBox().run();
    }
