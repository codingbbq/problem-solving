...
class ChefDiet {
    ...
    void solve(int TC) throws Exception {
        ...
        for(int i = 0; i < N; i++) {
            int totalForDay = A[i] + remain;
            if(totalForDay >= K) {
                remain = totalForDay - K;
            } else {
                protein = false;
                day = i+1;
                break;
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new ChefDiet().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new ChefDiet().run();
    }
