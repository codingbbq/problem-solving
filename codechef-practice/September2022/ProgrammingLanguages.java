...
class ProgrammingLanguages {
    ...
    void solve(int TC) throws Exception {
        int A = ni(), B = ni(), A1 = ni(), B1 = ni(), A2 = ni(), B2 = ni();
        int found = 0;
        if(A == A1) {
            if(B == B1) {
                found = 1;
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new ProgrammingLanguages().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new ProgrammingLanguages().run();
    }
