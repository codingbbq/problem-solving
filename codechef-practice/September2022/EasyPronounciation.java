...
class EasyPronounciation {
    ...
    void solve(int TC) throws Exception {
        ...
        for(int i = 0; i < N; i++) {
            if(vowels.containsKey(s.charAt(i))) {
                ...
                consonent++;
                count = Math.max(consonent, count);
            }
        }
        println(count >= 4 ? "NO" : "YES");
    }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new EasyPronounciation().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new EasyPronounciation().run();
    }
