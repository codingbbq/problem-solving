// https://www.codechef.com/submit/CHEFFAV
...
class FavoriteString  {
    ...
    void solve(int TC) throws Exception {
        int N = ni();
        String s = ns();
        int x = s.indexOf("code");
        int y = s.indexOf("chef");
        if(x >= 0 && y >= 0 && x < y){
            println("AC");
        } else {
            println("WA");
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new FavoriteString().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new FavoriteString().run();
    }
