...
class ChefBoredGames {

    void solve(int TC) throws Exception {
        int N = ni();
        println(checkOddCount(N));
    }

    int checkOddCount(int n) {
        int count = 0;
        for(int i = 1; i <= n; i++) {
            if(i % 2 != 0) {
                int K = n - i + 1;
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new ChefBoredGames().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new ChefBoredGames().run();
    }
