...
class PushTowerJump {
    ...
    void solve(int TC) throws Exception {
        ...
        for(int x : freq.keySet()) {
            max = Math.max(max, x + freq.get(x) - 1);
        }
        println(max);
    }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new PushTowerJump().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new PushTowerJump().run();
    }
