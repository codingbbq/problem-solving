...
class ChefAndString {
    ...
    void solve(int TC) throws Exception {
        String s = ns();
        if((s.substring(1) + s.charAt(0) + "").equals(s.charAt(s.length()-1) + "" + s.substring(0,s.length()-1))) {
            println("YES");
        } else {
            println("NO");
        }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new ChefAndString().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new ChefAndString().run();
    }
