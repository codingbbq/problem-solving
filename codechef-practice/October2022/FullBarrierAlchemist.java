// https://www.codechef.com/submit/PIPSQUIK
...
class FullBarrierAlchemist {
    ...
    void solve(int TC) throws Exception {
        int N = ni(), H = ni(), Y1 = ni(), Y2 = ni(), L = ni();
        int Barrier = 0;
        for(int i = 0; i < N; i++) {
            int t = ni();
            int b = ni();
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new FullBarrierAlchemist().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new FullBarrierAlchemist().run();
    }
