...
class Starters59HappyString {
    ...
    void solve(int TC) throws Exception {
        ...
        for(int i = 1; i < s.length() - 1; i++) {
            if(v.containsKey(s.charAt(i-1)) && v.containsKey(s.charAt(i)) && v.containsKey(s.charAt(i+1))) {
                possible = true;
                break;
            }
        }
        println(possible ? "Happy" : "Sad");
    }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new Starters59HappyString().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new Starters59HappyString().run();
    }
