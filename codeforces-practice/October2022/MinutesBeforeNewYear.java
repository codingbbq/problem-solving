...
public class MinutesBeforeNewYear {
    ...
    void solve(int TC) throws Exception {
        int H = ni(), M = ni();
        int minutes = 60 - M; // Minutes required
        int hours = (23 - H) * 60; // Hours required
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new MinutesBeforeNewYear().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new MinutesBeforeNewYear().run();
    }
