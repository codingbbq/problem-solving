...
public class WordGame {
    ...
    void solve(int TC) throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            int tot = 0;
            for (int j = 0; j < N; j++) {
                if (mp.get(a[i][j]) == 1) {tot += 3;}
                else if (mp.get(a[i][j]) == 2) {tot++;}
            }
            print(tot + " ");
        }
        println("");
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new WordGame().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new WordGame().run();
    }
