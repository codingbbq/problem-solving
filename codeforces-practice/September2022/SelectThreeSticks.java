...
public class SelectThreeSticks {
    ...
    void solve(int TC) throws Exception {
        ...
        for(int i = 1; i < N - 1; i++) {
            int total = (A[i] - A[i-1]) + (A[i+1] - A[i]);
            mind = Math.min(mind, total);
        }
        println(mind);
    }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new SelectThreeSticks().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new SelectThreeSticks().run();
    }
