...
public class GiftsFixing {
    ...
    void solve(int TC) throws Exception {
        ...
        long count = 0;
        for(int i = 0; i < N; i++) {
            count += Math.max(Math.abs(minA - A[i]), Math.abs(minB - B[i]));
        }
    ...
    public static void main(String[] args) throws Exception{
        if(memory) new Thread(null, new Runnable() {public void run(){try{new GiftsFixing().run();}catch(Exception e){e.printStackTrace();}}}, "1", 1 << 28).start();
        else new GiftsFixing().run();
    }
